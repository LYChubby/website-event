document.addEventListener("DOMContentLoaded", () => {
            ...
            fetch("/categories")
                ...
                .then((categories) => {
                    ...
                    categories.forEach((cat, index) => {
                        ...
                        row.innerHTML = `
                            <td class="py-4 px-2 text-gray-600 dark:text-gray-400 font-medium">${index + 1}</td>
                            <td class="py-4 px-2">
                            ...
                            <td class="py-4 px-2">
                                <div class="flex space-x-2">
                                    <button onclick="editCategory(${cat.category_id}, '${cat.name}')" 
            ...
            fetch("/admin/events")
                ...
                .then((response) => {
                    ...
                    response.data.forEach((ev) => {
                        ...
                        card.className = "bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden card-hover shadow-lg";

                        const statusClass = ev.status_approval === 'pending' ? 'status-pending' : 
                                           ev.status_approval === 'approved' ? 'status-approved' : 'status-rejected';
                        ...
                        card.innerHTML = `
                            ...
                            <div class="p-6">
                                <h3 class="font-bold text-lg text-gray-800 dark:text-gray-200 mb-2 line-clamp-2">${ev.name_event}</h3>
                                
                                <div class="flex items-center mb-3">
                                    <i class="fas fa-tag text-gray-400 mr-2"></i>
                                    <span class="text-sm text-gray-600 dark:text-gray-400">${ev.category?.name ?? "-"}</span>
                                </div>
                                
                                <div class="flex items-center mb-3">
                                    <i class="fas fa-calendar text-gray-400 mr-2"></i>
                                    <span class="text-sm text-gray-600 dark:text-gray-400">${ev.start_date} - ${ev.end_date}</span>
                                </div>
                                
                                <div class="flex items-center justify-between mb-4">
                                    <span class="status-badge ${statusClass}">${ev.status_approval}</span>
                                </div>
                                
                                <div class="flex space-x-2">
                                    <button onclick="approveEvent(${ev.event_id})" 
                                    ...
                                    <button onclick="rejectEvent(${ev.event_id})" 
        ...
        function updateStats() {
            ...
            fetch("/admin/events")
                ...
                .then(response => {
                    if (response.data) {
                        const approved = response.data.filter(e => e.status_approval === 'approved').length;
                        const pending = response.data.filter(e => e.status_approval === 'pending').length;
        ...
        window.openCategoryModal = function (mode = "create") {
            ...
            if (mode === "create") {
                ...
                document.getElementById("categoryModalTitle").innerHTML = '<i class="fas fa-plus mr-3"></i>Tambah Kategori';
            } else if (mode === "edit") {
                document.getElementById("categoryModalTitle").innerHTML = '<i class="fas fa-edit mr-3"></i>Edit Kategori';
            }
