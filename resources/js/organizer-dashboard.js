document.addEventListener("DOMContentLoaded", () => {
            ...
            fetch("/organizer/events")
                ...
                .then((response) => {
                    ...
                    const events = categoryId === "all"
                        ? response.data
                        : response.data.filter((ev) => ev.category_id == categoryId);
                        ...
                        card.innerHTML = `
                            ...
                            <div class="event-content">
                                ...
                                <div class="organizer-info">
                                    <div class="organizer-avatar">${getInitials(ev.organizer_name || "Organizer")}</div>
                                    <span class="organizer-name">${ev.organizer_name || "Organizer"}</span>
                                </div>
                                <div class="card-actions">
                                    <button class="action-btn edit-btn" onclick="editEvent(${ev.event_id})">
                                        <i class="fas fa-edit"></i> Edit
                                    </button>
                                    <button class="action-btn delete-btn" onclick="deleteEvent(${ev.event_id})">
            ...
            fetch(url, {
                ...
                .then((response) => {
                    if (response.success || response.data) {
                        ...
                        showNotification(isEdit ? "Event berhasil diperbarui!" : "Event berhasil ditambahkan!", "success");
                    } else {
                ...
                .catch(error => {
                    ...
                    showNotification("Gagal menyimpan event. Silakan coba lagi.", "error");
                })
                .finally(() => {
            ...
            if (confirm("Yakin ingin menghapus event ini?")) {
                ...
                .then((response) => {
                    if (response.success) {
                        ...
                        throw new Error(response.message || "Gagal menghapus event");
                    }
                })
                .catch(error => {
                    console.error("Error deleting event:", error);
                    showNotification("Gagal menghapus event. Silakan coba lagi.", "error");
                });
            }
        };
        ...
        let draftTimeout;
        const formInputs = document.querySelectorAll('#eventForm input, #eventForm textarea, #eventForm select');
        ...
        function loadDraft() {
            ...
            if (draft && !document.getElementById('eventId').value) { // Only load for new events
        ...
        function toggleMobileMenu() {
            ...
            headerControls.style.display = headerControls.style.display === 'none' ? 'flex' : 'none';
        }
